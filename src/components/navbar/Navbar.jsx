...
const Navbar = () => {
  ...
  let handleSearch = (data1) => {
    return data1?.filter((product) =>
      product.title.toLowerCase().includes(search.toLowerCase())
    );
  };
  console.log(handleSearch(data1));
  useEffect(() => {
    axios
      .get("https://dummyjson.com/products")
      .then((res) => setData1(res.data.products));
  }, []);
  return (
